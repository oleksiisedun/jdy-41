...
- **`protocol.js`** — pure wire-format helpers, factored out of `index.js` so they're unit-testable without a device: `validateParams` (hex-byte gatekeeper), `getBuffer` (hex string → `Buffer`), `getInstruction` (assembles `<head> [params...] <terminator>`, terminator `0D 0A`), `toHexBytes` (hex string → uppercase byte strings), `appendChunk`/`isComplete` (accumulate serial data chunks of any size and detect the terminator), and `responseToString` (decodes an accumulated response, see below).
